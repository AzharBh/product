...
public class JWTService {
	...
	public Authentication getAuthorization(String token) throws Exception {
	...
	private Authentication getUser(String encodedSecret, String token) throws Exception {
	...
	private String getToken(String encodedSecret, UserBean jwtUser) {
		Date now = new Date();
		return Jwts.builder().setId(UUID.randomUUID().toString()).setSubject(jwtUser.getEmail())
				.claim("roles", jwtUser.getEmail()).setIssuedAt(now).setExpiration(getExpirationTime())
				.signWith(SignatureAlgorithm.HS512, encodedSecret).compact();
	}

	public String getToken(UserBean jwtUser) {
